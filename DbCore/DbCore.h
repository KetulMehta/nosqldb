...
namespace NoSqlDb
{
  ...
  public:
	...
	void addtoDb(std::string dbkey, DbElement<std::string> dbelement)
	{
		this->dbStore_[dbkey] = dbelement;
	}

	// Deletion from database

	void deletefromDb(std::string dbkey, DbElement<std::string> dbelement)
	{
		iterator it = dbStore_.find(dbkey);
		dbStore_.erase(it);
	}

	// Updation on Database

	void updateDb(std::string dbkey, DbElement<std::string> dbelement)
	{
		this->dbStore_[dbkey] = dbelement;
	}

	// Deletion of Child from database
	
	void deletechild(std::string dbkey,std::string childkey) {
		std::vector<std::string> vectorA = dbStore_[dbkey].children();
		for ( unsigned int i = 0; i < vectorA.size(); i++)
		{
			if (vectorA[i]==childkey)
			{
				vectorA.erase((vectorA.begin())+i);
			}
		}
		dbStore_[dbkey].children()=vectorA;
	}
